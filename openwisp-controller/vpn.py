...
from ..api.zerotier_api import ZeroTierAPI
...
class AbstractVpn(ShareableOrgMixinUniqueName, BaseConfig):
    ...
    # using this for rest api token for zerotier
    ...
    def save(self, *args, **kwargs):
        ...
        if self._is_backend_type('zerotier'):
            if not self.auth_token:
                raise ValidationError(
                    {'auth_token': _('ZeroTier auth token is needed')}
                )
            if self.config.get('id', None) is None:
                response = ZeroTierAPI(self.host, self.auth_token).create_network(
                    self.name
                )
